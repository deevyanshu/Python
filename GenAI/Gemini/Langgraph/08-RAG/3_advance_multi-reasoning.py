...
# this is to maintain history for follow up questions and to rephrase the question to be more specific for retrieval
def question_rewriter(state: AgentState):
    print(f"Entering question_rewriter with following state: {state}")

    # Reset state variables except for 'question' and 'messages' because we want messages to continue to accumulate for the conversation history and question to be updated with the latest question
    state["documents"] = []
    state["on_topic"] = ""
    ...
    if len(state["messages"]) > 1:
        conversation = state["messages"][:-1] #extracting all messages except last question
...
res2=graph.invoke(input=input_data, config={"configurable": {"thread_id": 2}})
print(res2)

input_data = {
    "question": HumanMessage(content="Who founded Peak Performance Gym?")
}
res3=graph.invoke(input=input_data, config={"configurable": {"thread_id": 3}})
print(res3)

input_data = {"question": HumanMessage(content="When did he start it?")}
res4=graph.invoke(input=input_data, config={"configurable": {"thread_id": 3}})
print(res4)